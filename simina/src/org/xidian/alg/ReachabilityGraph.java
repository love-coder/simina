 ...
 * @version 1.1 version 2016-6-4 增加环的检测，traverseRG()方法中生成邻接矩阵
...
//		}
	...
	public String getDeadlockPathLength() {
		...
		resultStr.append("deadlock 状态：" + printSet(criticalNodes));
		resultStr.append("\n\n关系如下,s#(临界)==>s#(deadlock)\n");
		//下面开始打印关系
		...
		for(Integer cri : criticalNodes) {
			...
			for(Integer i : tempDownNodes) {
				if(preDeadlockNodes.contains((Integer)i)) {
					...
					//System.out.println("s"+(cri+1)+"==>"+"s"+(i+1));
				}
			}
		}
		
		//System.out.println("-----"+graphModel.getArcMatrix().getMatrix()[2][4]);
